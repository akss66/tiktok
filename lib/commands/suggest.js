...
async function cmdSuggest(ctx, args) {
  ...
  const skipHumanDelay = fast || process.env.NODE_ENV === 'test' || process.env.VITEST === 'true';
  ...
  // 模拟阅读时间（概率 50%）
  if (!skipHumanDelay) await maybeDelay(0.5, 2000, 6000);
  ...
  // 模拟思考时间（概率 30%）
  if (!skipHumanDelay && suggestions.length > 0) await maybeDelay(0.3, 3000, 8000);
  ...
  if (dupCids.size > 0) {
    ...
    for (const s of suggestions) {
      ...
      try {
        const srcText = (toReply.find(t => t.cid === s.cid) || {}).text || '';
        let newReply = '';
        if (typeof client.rewriteReply === 'function') {
          newReply = await client.rewriteReply(srcText, s.reply, strategy, '', persona);
        } else {
          const rewritten = await client.suggestReplies([{ cid: s.cid, text: srcText }], strategy, '', {
            ...llmContext,
            avoid: [...(llmContext.avoid || []), s.reply],
          }, persona);
          newReply = String(rewritten?.[0]?.reply || '').trim();
        }
