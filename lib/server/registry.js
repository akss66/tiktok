...
const DEFAULT_HTTP_MAX_IDLE_MS = 45000;
...
class ConnectionRegistry {
  ...
  get(site, index = 0) {
    ...
    const httpAlive = conns.filter(c => this.isAlive(c));
    return httpAlive[Math.min(index, httpAlive.length - 1)] || null;
  }
  ...
  isAlive(conn, now = Date.now(), httpMaxIdleMs = DEFAULT_HTTP_MAX_IDLE_MS) {
    if (!conn) return false;
    if (conn.ws) return conn.ws.readyState === 1;
    const last = Date.parse(conn.lastActivity || conn.connectedAt || '');
    if (!Number.isFinite(last)) return false;
    return now - last <= httpMaxIdleMs;
  }
  ...
  list(options = {}) {
    ...
    const httpMaxIdleMs = options.httpMaxIdleMs || DEFAULT_HTTP_MAX_IDLE_MS;
    const result = {};
    for (const [site, conns] of this._map.entries()) {
      result[site] = conns.map(c => ({
        ...
        alive: this.isAlive(c, now, httpMaxIdleMs),
      }));
    }
    return result;
  }
  ...
  countAlive(options = {}) {
    ...
    let count = 0;
    for (const conns of this._map.values()) {
      count += conns.filter(c => this.isAlive(c, now, httpMaxIdleMs)).length;
    }
    return count;
  }
