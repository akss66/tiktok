...
class Router {
  ...
  _status(res) {
    ...
    res.end(JSON.stringify({
      ...
      totalAliveConnections: this.registry.countAlive(),
      pollWaiters: this._mapCounts(this._pollWaiters),
      pollQueue: this._mapCounts(this._pollQueue),
      uptime: Math.floor(process.uptime()),
    }));
  }
  ...
  _mapCounts(map) {
    const counts = {};
    for (const [site, items] of map.entries()) {
      counts[site] = Array.isArray(items) ? items.length : 0;
    }
    return counts;
  }
  ...
  async _poll(url, res) {
    ...
    if (connId) this.registry.touch(connId);
