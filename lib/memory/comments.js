...
/**
 * 清理 last_seen 早于 ts 的评论（TTL）。dryRun 只计数。返回删除/命中数。
 */
function cleanupBefore(ts, opts = {}) {
  try {
    const db = getDb();
    if (opts.dryRun) {
      return db.prepare('SELECT count(*) AS n FROM comments WHERE platform = ? AND last_seen < ?').get(PLATFORM, ts).n;
    }
    const limit = opts.limit || 1000;
    return db.prepare('DELETE FROM comments WHERE platform = ? AND last_seen < ? LIMIT ?').run(PLATFORM, ts, limit).changes;
  } catch (e) {
    if (process.env.DOUYIN_DEBUG) console.warn('[comments.cleanupBefore] failed:', e.message);
    return 0;
  }
}
...
function listReplied(filters = {}, opts = {}) {
  try {
    ...
    const params = [PLATFORM];
    if (filters.awemeId) { where.push('aweme_id = ?'); params.push(String(filters.awemeId)); }
    ...
    const rows = db.prepare(`
      SELECT cid, aweme_id, reply_cid, last_seen
      FROM comments
    ...
    if (process.env.DOUYIN_DEBUG) console.warn('[comments.listReplied] failed:', e.message);
    return [];
  }
}
...
module.exports = {
  ...
  listByUid, listByVideo, listReplied, count, cleanupBefore, hashText, normalizeText,
};
